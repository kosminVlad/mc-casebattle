...
const Button: React.FC<React.PropsWithChildren<{
  ...
  const baseStyle = "font-bold rounded-xl transition-all duration-300 transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap";
  ...
  const variants = {
    primary: "bg-mc-accent-emerald text-white hover:bg-mc-accent-emerald/90 shadow-lg shadow-emerald-500/25",
    secondary: "bg-mc-bg-secondary text-mc-text-primary hover:bg-white/10 border border-white/10",
    danger: "border border-red-900/50 text-red-400 hover:bg-red-900/20 hover:border-red-500 bg-slate-900/50"
  };

  return (
    <button className={`${baseStyle} ${sizes[size]} ${variants[variant]} ${className}`} onClick={onClick}>
      {children}
    </button>
  );
};

// Тот самый Header из CasePage.tsx
const Header: React.FC<{
  balance: number;
  username: string;
  isConnected: boolean;
  onConnect: () => void;
}> = ({ balance, username, isConnected, onConnect }) => {
  return (
    <header className="sticky top-0 z-50 bg-mc-bg-primary/90 backdrop-blur-sm shadow-md border-b border-white/10 px-6 py-4">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        <div className="flex items-center space-x-2 cursor-pointer">
          <div className="text-2xl font-extrabold text-white">MC<span className="text-mc-accent-emerald">Cases</span></div>
        </div>

        <div className="flex items-center space-x-4">
          <div className="flex items-center bg-mc-bg-secondary px-4 py-2 rounded-xl text-mc-accent-emerald font-bold border border-white/10">
            💰 {balance.toLocaleString()} MC-Coins
          </div>
          
          {isConnected ? (
            <div className="flex items-center space-x-2">
              <span className="text-mc-text-secondary hidden md:inline">Welcome,</span>
              <span className="font-bold text-mc-accent-emerald">{username}</span>
              <div className="w-3 h-3 bg-mc-accent-emerald rounded-full animate-pulse" title="Connected"></div>
            </div>
          ) : (
...
export default function UserProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-main text-mc-text-primary font-sans overflow-x-hidden">
        <style jsx global>{`
            /* Core Styles imported from CasePage logic */
            .bg-gradient-main {
                background: #0b0e13;
            ...
            <section>
                ...
                {inventory.length === 0 && (
                    <div className="text-center py-20 text-mc-text-secondary bg-white/5 rounded-2xl border-2 border-dashed border-white/10">
                        ...
                        <Button variant="primary" className="mt-4" onClick={() => router.push('/')}>Перейти к кейсам</Button>
                    </div>
                )}
            </section>
        </main>
